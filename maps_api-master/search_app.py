...
def main():
    global router_mode, x1, x2, y1, y2, pnts, points, delta_x, delta_y
    pygame.init()
    ...
    while True:
        ...
        if event.type == pygame.QUIT:
            ...
            delta_x, delta_y = delta(event.key, delta_x, delta_y)        
            mp.update(event)
        elif event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:
                router_mode = True            
                x1, y1 = event.pos
                points.append([(x1 + 10 * delta_x) * (zoom - 16), (y1 + 10 * delta_y) * (zoom - 16)])
